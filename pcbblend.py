...
class PCBImport(bpy.types.Operator):
    ...
    board_top       = %BOARDTOP%
    board_bottom    = %BOARDBOTTOM%
    
    trimmed_height  = page_height - board_top
    board_height    = board_bottom - board_top
    
    offset_x        = %BOARDLEFT%
    offset_y        = trimmed_height - board_height
    #11.3
...
%DNPLIST%
    ...
    def populate(self):
        ...
        for id, name, value, x, y, rot, side in layout_table:
            ...
            frot = float(rot)
            try:
                if self.rotations[id]:
                    frot = self.rotations[id]
                    print("Rotation " + id + " : " + frot)
            except:
                print("Fail: " + id)
                pass
            try:
